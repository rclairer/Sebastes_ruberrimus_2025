...
copy_SS_inputs(
  dir.old = model_2017_path,
  dir.new = file.path(getwd(), "model", "2025_updated_catch_WCGBTS"),
  create.dir = FALSE,
  overwrite = TRUE,
  use_ss_new = TRUE,
  verbose = TRUE
...
#################################
# Catch and WCGBTS Updated Model #
################################

inputs_catch_WCGBTS <- SS_read(dir = file.path(getwd(), "model", "2025_updated_catch_WCGBTS"))
inputs_catch_WCGBTS$dat$endyr <- 2024
...
CA_1981_2015_TWL <- inputs_catch_WCGBTS$dat$catch |>
  filter(fleet == 1) |>
  filter(year > 1980 & year < 2016)
...
CA_1981_2015_NONTWL <- inputs_catch_WCGBTS$dat$catch |>
  filter(fleet == 2) |>
  filter(year > 1980 & year < 2016)
...
CA_missing_1981_2004_rec <- inputs_catch_WCGBTS$dat$catch |>
  filter(fleet == 3 & year > 1980 & year < 2005) |>
  filter(!year %in% CA_1981_2004_REC$year)
...
inputs_catch_WCGBTS$dat$catch <- all_catch
...
SS_write(inputs_catch_WCGBTS, dir = file.path(getwd(), "model", "2025_updated_catch_WCGBTS"), overwrite = TRUE)

get_ss3_exe(dir = file.path(getwd(), "model", "2025_updated_catch_WCGBTS"))

run(dir = file.path(getwd(), "model", "2025_updated_catch_WCGBTS"), show_in_console = TRUE)
replist <- SS_output(dir = file.path(getwd(), "model", "2025_updated_catch_WCGBTS"))
SS_plots(replist)
