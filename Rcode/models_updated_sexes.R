...
copy_SS_inputs(
  ...
  dir.new = file.path(getwd(), "model", "2025_updated_catch_WCGBTS_sex"),
  create.dir = FALSE,
  overwrite = TRUE,
  use_ss_new = TRUE,
  verbose = TRUE
...
####################################
# Catch and WCGBTS and Sexes Model #
####################################

inputs_catch_WCGBTS_sex <- SS_read(dir = file.path(getwd(), "model", "2025_updated_catch_WCGBTS_sex"))
inputs_catch_WCGBTS_sex$dat$endyr <- 2024
inputs_catch_WCGBTS_sex$ctl$Nsexes <- -1
...
CA_1981_2015_TWL <- inputs_catch_WCGBTS_sex$dat$catch |>
  filter(fleet == 1) |>
...
CA_1981_2015_NONTWL <- inputs_catch_WCGBTS_sex$dat$catch |>
  filter(fleet == 2) |>
  filter(year > 1980 & year < 2016)
...
CA_missing_1981_2004_rec <- inputs_catch_WCGBTS_sex$dat$catch |>
...
inputs_catch_WCGBTS_sex$dat$catch <- all_catch
...
inputs_catch_WCGBTS_sex$dat$indices <- inputs_catch_WCGBTS_sex$dat$CPUE |>
  filter(index != 11) |>
  bind_rows(NWFSC_ORWA_index) |>
  arrange(index, year)

SS_write(inputs_catch_WCGBTS_sex, dir = file.path(getwd(), "model", "2025_updated_catch_WCGBTS_sex"), overwrite = TRUE)

get_ss3_exe(dir = file.path(getwd(), "model", "2025_updated_catch_WCGBTS_sex"))

run(dir = file.path(getwd(), "model", "2025_updated_catch_WCGBTS_sex"), show_in_console = TRUE)
replist <- SS_output(dir = file.path(getwd(), "model", "2025_updated_catch_WCGBTS_sex"))
SS_plots(replist)
