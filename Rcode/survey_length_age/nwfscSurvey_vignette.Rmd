...
raw_length_comps <- raw_length_comps_n_trips |>
  ...
  dplyr::mutate(
    ...
    Nsamp = ifelse(n_samps_per_tow < 55, (n_tows + 0.0707*n_samps), 4.89*n_tows)
    ) |>
  dplyr::select(year, month, fleet, sex, partition, Nsamp, everything(), -n_samps_per_tow, -n_samps, -n_tows) |>
  write.csv(file.path(dirname(dirname(getwd())), "Data", "processed", "NWFSC.Combo_and_Tri_length_comps", "NWFSC.Combo_length_cm_unsexed_raw_10_74_yelloweye rockfish_groundfish_slope_and_shelf_combination_survey.csv"), row.names = FALSE)
```
...
raw_age_comps <- raw_age_comps_n_trips |>
  ...
  dplyr::select(year, month, fleet, sex, partition, Nsamp, everything(), -n_samps_per_tow, -n_samps, -n_tows) |>
  write.csv(file.path(dirname(dirname(getwd())), "Data", "processed", "NWFSC.Combo_age_comps", "Tri_length_cm_unsexed_raw_10_74_yelloweye rockfish_groundfish_triennial_shelf_survey.csv"))
...
raw_length_comps <- raw_length_comps_n_trips |>
  ...
  mutate(
    n_samps_per_tow = n_samps / n_tows,
    Nsamp = ifelse(n_samps_per_n_tows < 55, (n_tows + 0.0707 * n_samps), 4.89*n_tows)
    ) |>
  dplyr::select(year, month, fleet, sex, partition, Nsamp, everything(), -n_samps_per_tow)
