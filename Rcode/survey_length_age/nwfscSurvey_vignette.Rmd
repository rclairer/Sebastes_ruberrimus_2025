...
raw_length_comps <- raw_length_comps_n_trips |>
  ...
  dplyr::mutate(
    ...
    Nsamp = ifelse(n_samps_per_tow < 55, (n_tows + 0.0707 * n_samps_per_tow), 4.89*n_tows)
    ) |>
...
raw_length_comps <- raw_age_comps_n_trips |>
  left_join(raw_age_comps_n_fish, by = "year") |>
  mutate(
    n_samps_per_tow = n_samps / n_tows,
    Nsamp = ifelse(n_samps_per_n_tows < 55, (n_tows + 0.0707 * n_samps_per_tow), 4.89*n_tows)
    ) |>
  dplyr::select(year, month, fleet, sex, partition, Nsamp, everything(), -n_samps_per_tow)
