...
bds_cleaned <- cleanPacFIN(
  ...
  keep_gears = c(used_gears,'POT','NET','TLS'),
  CLEAN = TRUE,
...
#age
wcgop_caal <- read.csv(file = '../../Data/raw/nonconfidential/wcgop/biological_discard_caal.csv') %>% 
...
write.csv(raw_length_comps_comb,row.names = FALSE,
          file=paste(file_path,"Commercial_length_comps_PacFIN_WCGOP_forSS.csv"))
```

pb with 1978 and 1989 for CA.TWL -> no value (I think FTID is NA so no included in effort computation) -> to sort our later -> use value from 2017 in the between
...
write.csv(raw_age_caal_comb,row.names = FALSE,
          file = paste0(file_path,'/Commercial_caal_PacFIN_WCGOP.csv'))
```
...
### Add WCGOP data
 ...
 WCGOP are caal needs to be transform to maal -> just seting length bin lo/hi to -1 and summing number by year*fleet
...
raw_age_comps_comb <- raw_age_comps  %>%  
  ...
  mutate(ageerr=1,#WDFW is aging everything
         Lbin_lo=-1,Lbin_hi=-1,
         fleet_nb=-fleet_nb) #this is ghost comp, - is telling the model
...
write.csv(raw_age_comps_comb,row.names = FALSE,
          file=paste(file_path,"Commercial_age_comps_PacFIN_WCGOP_forSS.csv"))
```
