...
bds_cleaned <- cleanPacFIN(
  ...
  spp = "yelloweye") |>
  mutate(geargroup = ifelse(geargroup=='HKL','NONTWL',geargroup))|>
  dplyr::mutate(
...
```{r}
#length
wcgop_length_comps <- read.csv(file = '../../Data/raw/nonconfidential/wcgop/biological_discard_lengths_raw.csv') %>% 
...
#sample size
wcgop_sample_size <- read.csv(file = '../../Data/raw/nonconfidential/wcgop/wcgop_biological_sample_sizes.csv') %>% 
...
#age
wcgop_age_comps <- read.csv(file = '../../Data/raw/nonconfidential/wcgop/biological_discard_caal.csv') %>% 
...
WCGOP_length_sample_effort %>% 
              mutate(input_n2= round(ifelse(ratio_lengths <44,
...
-> can also use PacFINtools::getComps() to double check -> not done so far
...
raw_length_comps_comb <- raw_length_comps  %>%  bind_rows(wcgop_length_comps) %>% 
  group_by(year,month,fleet_nb,sex,partition) %>% 
  summarise_at(.vars=vars(input_n:u74),.funs=sum)

raw_length_comps_comb
