...
age_sample_effort <- bds_cleaned_addCAage %>%
  ...
  filter(!is.na(lengthcm)) %>%  #remove fish w no length, cause we need them to produce caal
...
# looping over every fleet
for (f in 1:nrow(fleets_names)){
  tmp_dat <- bds_cleaned_addCAage %>% filter(!is.na(Age),!is.na(lengthcm),stratification_byarea==as.character(fleets_names[f,1])) %>% 
     mutate(common_name='yelloweye rockfish') %>% 
    mutate(lengthcm=ifelse(is.na(lengthcm),length/10,lengthcm))
