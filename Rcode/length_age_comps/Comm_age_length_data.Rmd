...
```{r, data loading}
# getwd()
load('../../Data/raw/confidential/PacFIN.YEYE.bds.21.Mar.2025.RData') #biological dataset updated
load('../../Data/raw/confidential/PacFIN.YEYE.CompFT.21.Mar.2025.RData') #landings
...
raw_age_comps_comb <- raw_age_comps  %>%  
  bind_rows(wcgop_caal %>%  #filtering out CA_TWL sample in 2005 bc not used in 2017
            filter(!(fleet_nb==1&year==2005)) %>% mutate(Lbin_lo=-1,Lbin_hi=-1)) %>% 
  group_by(year,month,fleet_nb,sex,partition,ageerr,Lbin_lo,Lbin_hi) %>% 
