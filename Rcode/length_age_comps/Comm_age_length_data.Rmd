...
```{r}
fleets_names <- data_frame(fleet_name=c( "CA.TWL","CA.HKL","ORWA.HKL","ORWA.TWL"),fleet_nb=c(1,2,4,5))
```
...
```{r}
length_sample_size <- length_sample_effort %>%
...
tmp_plt <- raw_age_caal_long %>%
  ...
  # ggsave(tmp_plt,filename=paste0("Commercial_",fleets_names[f,1],'_length_compsDIFF.png'),
  #        width=10)
...
for (f in 1:nrow(fleets_names)){
  ...
  tmp_plt <- raw_age_comps_long %>% 
    ...
    ggsave(tmp_plt,filename=paste0("Commercial_",fleets_names[f,1],'_age_compsCOMP_no0.png'),
           width=10)
  }
