...
df1 <- read.csv('~/Git/Sebastes_ruberrimus_2025/Data/raw/confidential/recfin_WDFW_length_age.csv')

df1 %>%
  rename(
    year = SAMPLE_YEAR,
    common_name = RECFIN_SPECIES_NAME,
    sex = RECFIN_SEX_CODE,
    age = USE_THIS_AGE
  )

df %>%
  mutate(length = RECFIN_LENGTH_MM / 10)

df1 %>%
  mutate(length = RECFIN_LENGTH_MM / 10)

or_mrfss <- read_csv("C:/Morgan's Stuff/Sebastes_ruberrimus_2025/Data/raw/confidential/MRFSS_BIO_2025 CYCLE_457.csv")
recfin_wa_age <- read.csv("C:/Morgan's Stuff/Sebastes_ruberrimus_2025/Data/raw/confidential/SD506.csv")
```
...
# We might just need to set a cut off date: anything before 2003, we use old data, 2003 and newer we use new data that we have?


CA_REC: Recfin, and Don Pearson data (lengths come with the ages)

#Problems: recfin data only goes back to 2004, I bet Don Pearson's data goes back to 1979
  Can we just use 2017 data from 1979-2004?
  
#for now, just compare current recfin 2004-2024 to old assessment 2004-2017
#look at 2020 data just in case
...
OR_REC: MRFSS and ORBS combined, 
plus lengths with WDFW ages (1979-2012) or 2002????  *Fabio??*
ODFW ages (2009-2016/present?) *Ali has this and will upload*
