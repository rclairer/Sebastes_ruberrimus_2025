...
wa_data <- read.csv("/Users/abby/Git/Sebastes_ruberrimus_2025/Data/raw/confidential/SD501--1983---2024.csv")
...
#rename wa columns to match
names(wa_data)[names(wa_data) == "RECFIN_YEAR"] <- "year"
names(wa_data)[names(wa_data) == "SPECIES_NAME"] <- "common_name"
names(wa_data)[names(wa_data) == "RECFIN_SEX_CODE"] <- "sex"
names(wa_data)[names(wa_data) == "STATE_NAME"] <- "state"
names(wa_data)[names(wa_data) == "RECFIN_LENGTH_MM"] <- "length"
wa_data$length <- wa_data$length/10
wa_rec <- wa_data %>% filter(state == "WASHINGTON")

## Building Fleet 3 and Fleet 8:
ca_rec <- recfin_bio %>% filter(state == "CALIFORNIA")

  # Remove CPFV data to make fleets 3 and 8 ################TRY SORTING BY SOURCE CODE INSTERAD OF MODE NAME. WILL THIS FIX THE ISSUES?
...
# Save
write.csv(or_rec_lengths, file.path("C:/Morgan's Stuff/Sebastes_ruberrimus_2025/Data/processed/rec_comps/or_rec_lengths.csv"))

# Comparison plots for 2004-2024 showing that the 2017 OR_REC fleet excludes party/charter boats (CPFV)

  # Read new data
or_rec_lengths_new <- read.csv("C:/Morgan's Stuff/Sebastes_ruberrimus_2025/Data/processed/rec_comps/or_rec_lengths.csv") |>
  mutate(assessment = "New_Data") |>
  dplyr::select(year, assessment, L10:L74) |>
  tidyr::pivot_longer(cols = c(-year, -assessment), names_to = "length", values_to = "freq") |>
  dplyr::mutate(length = gsub("L", "", length)) |>
...
comparison_plot_or
  ...
  #Save plot
ggsave(plot = comparison_plot_or, "or_rec_length_comp_comparisons.png", path = file.path("C:/Morgan's Stuff/Sebastes_ruberrimus_2025", "Rcode", "length_age_comps"))
...
Data from 2016-2024 obtainted from RECFIN, previous data copied over from assessment
...
# Save
write.csv(wa_rec_lengths, file.path("/Users/abby/Git/Sebastes_ruberrimus_2025/Data/processed/rec_comps/wa_rec_lengths.csv"))

# Comparison plots for 2004-2016

  # Read new data
wa_rec_lengths_new <- read.csv("/Users/abby/Git/Sebastes_ruberrimus_2025/Data/raw/confidential/wa_rec_lengths.csv") |>
  mutate(assessment = "New_Data") |>
  dplyr::select(year, assessment, L10:L74) |>
  tidyr::pivot_longer(cols = c(-year, -assessment), names_to = "length", values_to = "freq") |>
  dplyr::mutate(length = gsub("L", "", length)) |>
...
together_rec <- together_rec[together_rec$year >= 2004,]
comparison_plot_wa <- together_rec |>
  dplyr::filter(freq > 0) |>
  ggplot2::ggplot(aes(x = year, y = length, col = assessment, size = freq)) +
  ggplot2::geom_point(position = position_dodge(0.5)) +
...
comparison_plot_wa
  ...
  #Save plot
ggsave(plot = comparison_plot_wa, "/Users/abby/Git/Sebastes_ruberrimus_2025/Rcode/length_age_comps/wa_rec_length_comp_comparisons.png")
...
# Save
write.csv(ca_obs_lengths, file.path("C:/Morgan's Stuff/Sebastes_ruberrimus_2025/Data/processed/rec_comps/ca_obs_lengths.csv"))

# Comparison plots for 2004-2024 

  # Read new data
ca_obs_lengths_new <- read.csv("C:/Morgan's Stuff/Sebastes_ruberrimus_2025/Data/processed/rec_comps/ca_obs_lengths.csv") |>
  mutate(assessment = "New_Data") |>
  dplyr::select(year, assessment, L10:L74) |>
  tidyr::pivot_longer(cols = c(-year, -assessment), names_to = "length", values_to = "freq") |>
  dplyr::mutate(length = gsub("L", "", length)) |>
...
comparison_plot_ca
  ...
  #Save plot
ggsave(plot = comparison_plot_ca, "ca_obs_length_comp_comparisons.png", path = file.path("C:/Morgan's Stuff/Sebastes_ruberrimus_2025", "Rcode", "length_age_comps"))

# Next step is to combine all 2017 CA_obs data with New 2017-2024 data

```

testing script to try and figure out whats wrong with the early CA_obs data

```{r}
# #bring in old data so we can get nsamps 
# inputs_old <- r4ss::SS_read(dir = file.path("C:/Morgan's Stuff/Sebastes_ruberrimus_2025", "model", "2017_yelloweye_model_updated_ss3_exe"))
# ca_obs_lengths_old <- inputs_old$dat$lencomp
# ca_obs_lengths_old <- inputs_old$dat$lencomp |>
#   dplyr::filter(fleet == 8) |>
...
#   dplyr::select(year, assessment, Nsamp, `l10`:`l74`)
# 
# #remove data older than 2004 so we can bring it in
# ca_obs_lengths_old<-  ca_obs_lengths_old[ca_obs_lengths_old$year>=2004,]
# #also remove 2006...for some reason
# ca_obs_lengths_old<-  ca_obs_lengths_old[ca_obs_lengths_old$year!=2006,]
#   
# # now in the new data, get rid of data newer than 2016
# ca_obs_lengths<-  ca_obs_lengths[ca_obs_lengths$year<=2016,]
# 
# 
# ca_obs_lengths$input_n <- ca_obs_lengths_old$Nsamp
# 
#   # Read new data # with old nsamps
# ca_obs_lengths_new <- ca_obs_lengths |>
#   mutate(assessment = "New_Data") |>
#   dplyr::select(year, assessment, L10:L74) |>
#   tidyr::pivot_longer(cols = c(-year, -assessment), names_to = "length", values_to = "freq") |>
#   dplyr::mutate(length = gsub("L", "", length)) |>
...
# #   dplyr::select(year, assessment, Nsamp, `l10`:`l74`) 
# 
# ca_obs_lengths_old <- ca_obs_lengths_old|>
#   tidyr::pivot_longer(cols = c(-year, -assessment, -Nsamp), names_to = "length", values_to = "freq") |>
#   dplyr::mutate(length = gsub("^l", "", length)) |>
#   dplyr::group_by(year) |>
#   dplyr::mutate(
...
# comparison_plot_ca
# 
# ### This code shows that Nsamps doesnt have any affect on the bubble plots...still dont know why the data is so wrong
# 
# #look at the fish in 2006
# 
# s <- ca_rec[ca_rec$year==2006,]
# 
# #all the fish in 2006 are Retained and were caught on private/rental. so I dont think I am classifying the data wrong, I think the data from RecFin (from 2004-2008) is wrong, or at least different from what 2017 assessment used.
```
...
# Filter out everything before 2004 to avoid MRFSS data
or_obs_lengths <- or_obs_lengths[or_obs_lengths$year>=2004,]

# Save
write.csv(or_obs_lengths, file.path("C:/Morgan's Stuff/Sebastes_ruberrimus_2025/Data/processed/rec_comps/or_obs_lengths.csv"))

# Comparison plots for 2004-2024, 2023? 

  # Read new data
or_obs_lengths_new <- read.csv("C:/Morgan's Stuff/Sebastes_ruberrimus_2025/Data/processed/rec_comps/or_obs_lengths.csv") |>
  mutate(assessment = "New_Data") |>
  dplyr::select(year, assessment, L10:L74) |>
  tidyr::pivot_longer(cols = c(-year, -assessment), names_to = "length", values_to = "freq") |>
  dplyr::mutate(length = gsub("L", "", length)) |>
...
comparison_plot_or
  ...
  #Save plot
ggsave(plot = comparison_plot_or, "or_obs_length_comp_comparisons.png", path = file.path("C:/Morgan's Stuff/Sebastes_ruberrimus_2025", "Rcode", "length_age_comps"))
