...
wa_caal <- data.frame(wa_caal)
colnames(wa_caal) <- gsub("^unsexed\\.", "", colnames(wa_caal))
colnames(wa_caal) <- gsub("^f", "A", colnames(wa_caal))

write.csv(wa_caal,'~/Git/Sebastes_ruberrimus_2025/Data/processed/recfin_WDFW_caal_age.csv')

# Comparison plots
# WA ages
# Read new data
wa_caal_new <- read.csv('~/Git/Sebastes_ruberrimus_2025/Data/processed/recfin_WDFW_caal_age.csv') |>
  mutate(assessment = "Current") |>
  dplyr::select(year, assessment, A0:A65) |>
  tidyr::pivot_longer(cols = c(-year, -assessment), names_to = "age", values_to = "freq") |>
  dplyr::mutate(age = gsub("A", "", age)) |>
...
wa_caal_old <- inputs_old$dat$agecomp |>
...
together <- rbind(wa_caal_old, wa_caal_new)
comparison_plot_wa_age <- together |>
  dplyr::filter(freq > 0) |>
  ggplot2::ggplot(aes(x = year, y = age, col = assessment, size = freq)) +
  ggplot2::geom_point(position = position_dodge(0.5))
ggsave(plot = comparison_plot_wa_age, "recfin_wa_caal_comp_comparisons.png", path = file.path("/Users/abby/Git/Sebastes_ruberrimus_2025/Rcode/length_age_comps"))
