...
wa_ages_raw <- read_csv("../../Data/raw/confidential/SD506_1984_2024_march.csv")
  
  # Rename columns 
names(wa_ages_raw)[names(wa_ages_raw) == "SAMPLE_YEAR"] <- "year"
names(wa_ages_raw)[names(wa_ages_raw) == "RECFIN_SPECIES_NAME"] <- "common_name"
names(wa_ages_raw)[names(wa_ages_raw) == "RECFIN_SEX_CODE"] <- "sex"
names(wa_ages_raw)[names(wa_ages_raw) == "USE_THIS_AGE"] <- "age"
names(wa_ages_raw)[names(wa_ages_raw) == "RECFIN_LENGTH_MM"] <- "length"
  # Change the sex column to U and fix length
wa_ages_raw[,33] = "U"
wa_ages_raw$length <- wa_ages_raw$length/10
...
# Save
write.csv(or_rec_caal, file.path("../../Data/processed/rec_comps/or_rec_caal.csv"))
...
# Save
write.csv(wa_rec_caal, file.path("../../Data/processed/rec_comps/wa_rec_caal.csv"))

# Comparison plots for 1998-2024
  # Read new data
wa_rec_caal_new <- read.csv("../../Data/processed/rec_comps/wa_rec_caal.csv") |>
  mutate(assessment = "Current") |>
  dplyr::select(year, assessment, A0:A65) |>
  tidyr::pivot_longer(cols = c(-year, -assessment), names_to = "age", values_to = "freq") |>
  dplyr::mutate(age = gsub("A", "", age)) |>
...
# Save
write.csv(wa_rec_maal, file.path("../../Data/processed/rec_comps/wa_rec_maal.csv"))

# Comparison plots for 1998-2024
  # Read new data
wa_rec_maal_new <- read.csv("../../Data/processed/rec_comps/wa_rec_maal.csv") |>
  mutate(assessment = "Current") |>
  dplyr::select(year, assessment, A0:A65) |>
  tidyr::pivot_longer(cols = c(-year, -assessment), names_to = "age", values_to = "freq") |>
  dplyr::mutate(age = gsub("A", "", age)) |>
