...
Pacific Fishery Management Council. \pageref*{LastPage}{} pp.
...
\(\alpha = 7.183309 \cdot 10^{-6}\), and \(\beta = 3.244801\)
...
spatially-explicit data become available. Both growth and initial
recruitment parameters were estimated internally, while all other
biological parameters were fixed. The modeling period starts in the
first year of available catches from historical reconstructions (1889)
and the stock is assumed to be at an unfished equilibrium prior to that
time. No changes were made to the fleet structure of the model. Fishery
removals were divided among seven area- and sector-specific fleets.
Estimated discard mortality was added to landings and included in the
model as fleet-specific total removals. Length compositions for
...
\item
  ...
  \item
    ...
    included in the model are included in (\textbf{section?}) 2 (add
    reference code) but are summarized below.
  \item
  ...
  \item
    ...
    spawning output by very little (\textbf{?@fig-bridge5-comp2}).
  \item
    ...
    1920-1980 (\textbf{?@fig-bridge14-comp2}). This new data also
    changed the patter of the recruitment deviations significantly from
    1990-2024 (\textbf{?@fig-bridge14-comp11}). From 1990-2005
    ...
    recovery curve (\textbf{?@fig-bridge14-comp2}).
  \end{itemize}
\item
  Fleet structure
...
\item
  ...
  \item
    ...
    include the most recent data and did not change the model fit. The
    impact of updating the length-weight relationship is evaluated as a
    sensitivity.
  \end{itemize}
\item
  Recruitment
  ...
  \tightlist
  \item
    The bias adjustment ramp was updated. (Do we specifically need to
    say what changed??)
  \end{itemize}
\item
...
\item
  ...
  \item
    ...
    and assessment document (\textbf{Include link here to github}
  \end{itemize}
\end{itemize}
