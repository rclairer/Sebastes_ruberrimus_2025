...
catch_table <- all_catch |>
  ...
  dplyr::mutate(CA_TWL = round(CA_TWL,2), CA_NONTWL = round(CA_NONTWL,2), CA_REC = round(CA_REC, 2), ORWA_TWL = round(ORWA_TWL, 2), ORWA_NONTWL = round(ORWA_NONTWL, 2), OR_REC = round(OR_REC, 2), WA_REC = round(WA_REC, 2), WA_REC_numbers = round(WA_REC_numbers, 2) )|>
    dplyr::select(-WA_REC)|> #remove wa rec mt, so comment out if we also want WA rec catch in mt
      dplyr::mutate(total_catch = round(rowSums(across(CA_TWL:OR_REC), na.rm = TRUE), 2)) |> #also will have to make this to WA:REC
      dplyr::rename("CA TWL (mt)" = "CA_TWL", "CA NONTWL (mt)" = "CA_NONTWL", "CA REC (mt)" = "CA_REC", "ORWA TWL (mt)" = "ORWA_TWL", "ORWA NONTWL (mt)" = "ORWA_NONTWL", "OR REC (mt)" = "OR_REC", "WA REC (1000s of fish)" = "WA_REC_numbers", "Catch (mt)" = "total_catch") |> #would have to add back in WA REC (mt)" = "WA_REC"
